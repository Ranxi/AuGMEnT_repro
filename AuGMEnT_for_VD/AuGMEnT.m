...
%                 fprintf('Terminated!\tAT PHASE %d\t',pnoOut);
            ...
            if(pno==4 && (trialSucc||Qa(1)~=2))
                if(trialSucc)
                    ...
                    fprintf('\tWrong Action!\tF1:%f,F2:%f\t',F1,F2);
                    break;
                end
            end
...
w
    ...
    function compute_input(pno)
        ...
        cur_x_ins = seq_x_ins(pno,:) .* [1,x_resp] + 0.075*randn(1,inputsize);
        cur_x_ins(1) = seq_x_ins(pno,1);
        cur_x_on = seq_x_on(pno,:) .* [1,x_resp]+ 0.075*randn(1,inputsize);
        cur_x_on(1) = seq_x_on(pno,1);
        cur_x_off = seq_x_off(pno,:) .* [1,x_resp]+ 0.075*randn(1,inputsize);
        cur_x_off(1) = seq_x_off(pno,1);
    end

    function calculateNet(t)
        %parameter: pno, phase sequence number; t, time-step in this phase
        if(t > 1)
            cur_x_on = zeros(1,inputsize) + 0.075*randn(1,inputsize);
            cur_x_off = zeros(1,inputsize) + 0.075*randn(1,inputsize);
        end
        y_r = cur_x_ins * v_reg;
        y_r = 1./(1+exp(-y_r));
